...
<div class="max-w-full mx-auto">
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600 mt-1">Manage library book categories</p>
        </div>
...
{{-- Include Modal --}}
@include('admin.library.categories._form')
...
function loadCategories() {
    ...
    fetch(`{{ route('admin.library.categories.index') }}?${params}`, {
        headers: {
...
function renderCategoriesTable(categories) {
    ...
    tbody.innerHTML = categories.map((category, index) => {
        ...
        const createdAt = new Date(category.created_at).toLocaleDateString();
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex gap-2">
                        ...
                        <button onclick="toggleStatus(${category.id})" class="text-yellow-600 hover:text-yellow-900" title="Toggle Status">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function renderPagination(pagination) {
    ...
    infoDiv.innerHTML = `Showing ${start} to ${end} of ${pagination.total} categories`;
...
function editCategory(id) {
    ...
    fetch(`{{ route('admin.library.categories.index') }}/${id}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json'
        }
...
function saveCategory() {
    ...
    const url = id ? `{{ route('admin.library.categories.index') }}/${id}` : '{{ route('admin.library.categories.store') }}';
    const method = 'POST';
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors) {
                ...
                showAlert(data.message || 'Failed to save category', 'error');
            }
        }
    })
    .catch(error => {
...
function performForceDelete(id) {
    ...
    if (!confirm('⚠️ WARNING: This will PERMANENTLY delete this category. This action CANNOT be undone!\n\nAre you absolutely sure?')) {
        return;
    }
    
    fetch(`/admin/library/categories/${id}/force-delete`, {
        method: 'DELETE',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
    ...
    .then(data => {
        if (data.success) {
            ...
            showAlert(data.message || 'Failed to permanently delete category', 'error');
        }
    })
    .catch(error => {
        console.error('Error force deleting category:', error);
        showAlert('Failed to permanently delete category', 'error');
    });
}

function toggleStatus(id) {
    fetch(`{{ route('admin.library.categories.index') }}/${id}/toggle-status`, {
        method: 'POST',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
