...
<div class="max-w-full mx-auto">
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Publishers</h1>
            <p class="text-gray-600 mt-1">Manage library publishers</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody id="publishers-table-body" class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                            <div class="flex justify-center items-center">
                                ...
                                <span class="ml-2">Loading publishers...</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
{{-- Include Modal --}}
@include('admin.library.publishers._form')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Form submit handler
    document.getElementById('publisher-form').addEventListener('submit', function(e) {
        e.preventDefault();
        savePublisher();
    });
});
...
function loadPublishers() {
    ...
    fetch(`{{ route('admin.library.publishers.index') }}?${params}`, {
        headers: {
    ...
    .then(data => {
        if (data.success) {
            renderPublishersTable(data.data);
...
function renderPublishersTable(publishers) {
    ...
    if (publishers.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">No publishers found</td></tr>';
        return;
    }
    
    tbody.innerHTML = publishers.map((publisher, index) => {
        const statusBadge = publisher.is_active 
            ? '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Active</span>'
            : '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">Inactive</span>';
        
        const phone = publisher.phone || '-';
        const email = publisher.email || '-';
        const createdAt = new Date(publisher.created_at).toLocaleDateString();
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex gap-2">
                        @if(auth()->user()->hasAnyRole(['admin', 'manager', 'staff']))
                        <button onclick="editPublisher(${publisher.id})" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                        </button>
                        <button onclick="toggleStatus(${publisher.id})" class="text-yellow-600 hover:text-yellow-900" title="Toggle Status">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button onclick="deletePublisher(${publisher.id})" class="text-red-600 hover:text-red-900" title="Delete">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function renderPagination(pagination) {
    ...
    infoDiv.innerHTML = `Showing ${start} to ${end} of ${pagination.total} publishers`;
...
function openCreateModal() {
    ...
    document.getElementById('modal-title').textContent = 'Add Publisher';
...
function editPublisher(id) {
    ...
    fetch(`{{ route('admin.library.publishers.index') }}/${id}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json'
        }
...
function savePublisher() {
    ...
    const url = id ? `{{ route('admin.library.publishers.index') }}/${id}` : '{{ route('admin.library.publishers.store') }}';
    const method = 'POST';
    ...
    .then(data => {
        if (data.success) {
            ...
            if (data.errors) {
                ...
                showAlert(data.message || 'Failed to save publisher', 'error');
            }
        }
    })
    .catch(error => {
        ...
        <div id="delete-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
            <div class="relative mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mt-4">Delete Publisher</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500">Are you sure you want to delete this publisher?</p>
...
function performSoftDelete(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            showAlert(data.message || 'Failed to delete publisher', 'error');
        }
    })
    .catch(error => {
...
function performForceDelete(id) {
    ...
    if (!confirm('⚠️ WARNING: This will PERMANENTLY delete this publisher. This action CANNOT be undone!\n\nAre you absolutely sure?')) {
        return;
    }
    
    fetch(`/admin/library/publishers/${id}/force-delete`, {
        method: 'DELETE',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
    ...
    .then(data => {
        if (data.success) {
            ...
            showAlert(data.message || 'Failed to permanently delete publisher', 'error');
        }
    })
    .catch(error => {
        console.error('Error force deleting publisher:', error);
        showAlert('Failed to permanently delete publisher', 'error');
    });
}

function toggleStatus(id) {
    fetch(`{{ route('admin.library.publishers.index') }}/${id}/toggle-status`, {
        method: 'POST',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
