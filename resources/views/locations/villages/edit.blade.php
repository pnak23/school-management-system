...
<div class="py-6">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Edit Village</h1>
            <p class="text-gray-600 mt-1">Update village information</p>
        </div>
            ...
            <form action="{{ route('villages.update', $village) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <select id="filter_province" 
                        ...
                        @foreach($provinces as $province)
                            <option value="{{ $province->id }}" {{ $village->commune->district->province_id == $province->id ? 'selected' : '' }}>
                                {{ $province->name_en }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-5">
                    ...
                    <select id="filter_district" 
                        ...
                        @foreach($districts as $district)
                            <option value="{{ $district->id }}" data-province="{{ $district->province_id }}" {{ $village->commune->district_id == $district->id ? 'selected' : '' }}>
                                {{ $district->name_en }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-5">
                    ...
                    <select name="commune_id" 
                        ...
                        @foreach($communes as $commune)
                            <option value="{{ $commune->id }}" data-district="{{ $commune->district_id }}" {{ old('commune_id', $village->commune_id) == $commune->id ? 'selected' : '' }}>
                                {{ $commune->name_en }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" 
                           name="name_km" 
                           id="name_km" 
                           value="{{ old('name_km', $village->name_km) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentCommune = '{{ old("commune_id", $village->commune_id) }}';
    ...
    districtSelect.addEventListener('change', function() {
        ...
        allCommunes.forEach(option => {
            if (!selectedDistrict || option.dataset.district == selectedDistrict) {
                const newOption = option.cloneNode(true);
                if (newOption.value == currentCommune) {
                    newOption.selected = true;
                }
                communeSelect.appendChild(newOption);
            }
        });
    });
